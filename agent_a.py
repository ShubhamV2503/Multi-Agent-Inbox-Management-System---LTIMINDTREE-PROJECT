...
def get_email_body(payload):
    ...
    try:
        ...
        body = ""
    return clean_forwarded_text(body)

# ---------------- Clean forwarded text ----------------
def clean_forwarded_text(text):
    # Remove common forwarded headers
    cleaned = re.sub(r"[-]+ Forwarded message [-]+.*", "", text, flags=re.DOTALL | re.IGNORECASE)
...
def fetch_and_process_emails():
    ...
    if os.path.exists("config.json"):
        ...
        high_priority_list = config.get("HighPriorityEmails", [])
    else:
        labels_list, email_map, friends_list, high_priority_list = [], {}, [], []

    # Ensure "High Priority" exists in config labels
    if "High Priority" not in labels_list:
        labels_list.append("High Priority")
        config["Label"] = labels_list
    ...
    for msg in messages:
        ...
        # --- Priority Logic ---
        if sender_email in high_priority_list or attachment == "Yes":
            category = "High Priority"
        elif sender_email in friends_list:
            category = "Friends"
        elif sender_email in email_map and email_map[sender_email]:
            category = email_map[sender_email]
        else:
            category = categorize_email(subject + "\n" + body, labels_list)

        if category not in labels_list and category not in ["Friends", "High Priority"]:
            category = "Other"
